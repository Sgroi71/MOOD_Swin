...
from checkpoint import get_b16_config, get_l16_config
...
def set_checkpoint_for_finetune(model, args, ckpt):
    if 'vit' in args.model:
        # load finetune in modeling_vit.py
        return model

    # else:
    if ckpt.startswith('https'):
        state_dict = torch.hub.load_state_dict_from_url(
            ckpt, map_location='cpu', check_hash=True)
    else:
        state_dict = torch.load(ckpt, map_location='cpu')

    print("Load ckpt from %s" % ckpt)
    if 'model' in state_dict.keys():
        state_dict = state_dict['model']
    ...
    if 'beit' in args.model:
        if 'head.weight' in state_dict.keys() and args.nb_classes != state_dict['head.weight'].size(0):
            del state_dict['head.weight']
            del state_dict['head.bias']
            print("re-initialize fc layer")

        if args.rel_pos_bias and "rel_pos_bias.relative_position_bias_table" in state_dict.keys():
    ...
    load_state_dict(model, state_dict, prefix=args.model_prefix)
    ...
    if loss_scaler is not None:
        ...
        if args.resume:
            ...
            if 'model' in checkpoint.keys():
                ...
                model_without_ddp.load_state_dict(checkpoint['module'])
            else:
